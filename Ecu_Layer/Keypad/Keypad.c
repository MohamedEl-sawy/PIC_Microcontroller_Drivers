...
Std_ReturnType keypad_initialize(const keypad_t *_keypad_obj){
    ...
    uint8 rows_counter = ZERO, columns_counter = ZERO;
    if(NULL == _keypad_obj){
        ret = E_NOT_OK;
    }
    else{    
        for(rows_counter=ZERO; rows_counter<ECU_KEYPAD_ROWS; rows_counter++){
            ret = gpio_pin_intialize(&(_keypad_obj->keypad_row_pins[rows_counter]));
        }
        for(columns_counter=ZERO; columns_counter<ECU_KEYPAD_COLUMNS; columns_counter++){
...
Std_ReturnType keypad_get_value(const keypad_t *_keypad_obj, uint8 *value){
    Std_ReturnType ret = E_OK;
    uint8 l_rows_counter = ZERO, l_columns_counter = ZERO, l_counter = ZERO;
    uint8 column_logic = ZERO;
    if((NULL == _keypad_obj) || (NULL == value)){
        ret = E_NOT_OK;
    }
    else{    
        for(l_rows_counter=ZERO; l_rows_counter<=ECU_KEYPAD_ROWS; ++l_rows_counter){
            for(l_counter=1; l_counter<=ECU_KEYPAD_ROWS; l_counter++){
            ...
            __delay_ms(10);
            for(l_columns_counter=ZERO; l_columns_counter<ECU_KEYPAD_COLUMNS; l_columns_counter++){
